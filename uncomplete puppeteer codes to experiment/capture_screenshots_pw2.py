...
original_path=logs_dir+us_timestamp_str()
...
PAGE_LOAD_TIMEOUT = 50000  # Max time to wait for a page load
...
async def handle_page(page):
    ...
    if url_next=="about:blank":
        ...
        await page.close() 
    elif utils.canonical_url(url_next)==utils.canonical_url("https://www1.123movies.cafe/123movies/"):
        print("new tab has the same domain with the initial page.. skipping")
        await page.close()
    else:
      await page.screenshot(path="{}_TAB_{}.png".format(original_path,us_timestamp_str()))
      #await page_next.set_viewport_size({ 'width': 1280, 'height': 1024 })
    ...
    # await stealth_async(page)
    # await page.wait_for_load_state()
    # print("here in new page handler........")
    # await page.screenshot(path="{}_next_notstealth_left.png".format(original_path))     
...
async def capture_screenshot_pwright(url: str, path: Path, viewport_width: int, viewport_height: int) -> None:
    async with async_playwright() as p:
        ...
        print(tabCount)
#<a href="chrome-headless-test.html">Check for yourself</a>

  
        # #await page.click("text=feel free to challenge it",button='middle')
        ...
        # await page.screenshot(path="{}_next_stealth_left.png".format(path))
...
@click.option('--filename', default="{}".format(original_path))
